# ARARE AI Genspark用PRD 完全版 / 最終統合版

最終更新日: 2026-06-12

このファイルは、ARARE AIをGenspark AI Developerへ渡すための完全版PRDである。

以下の追加論点をすべて統合済み。

- 100店舗導入前提
- 店舗の既存電話番号を変えない転送導入モード
- 店舗ごとのAI受付番号 `aiIngressPhoneNumber`
- Twilio費用前提と100店舗運用を見据えた設計
- Twilio固定ではなく `VoiceProviderAdapter` によるProvider差し替え
- Twilio Conversation RelayをMVP第一候補
- OpenAI Realtime SIP / Vapi / Retell / Telnyx / Plivoの将来比較
- ReservationHoldの期限と承認SLA
- NGルールの構造化
- LINEシフト解析
- イケボ音声の本番方針とモック方針
- マルチテナント設計
- Platform Adminによる100店舗監視

Gensparkに渡す場合は、まずこのPRD全文を添付または貼り付ける。

長すぎる場合は、先に以下のファイルを貼り、その後この完全版PRDを追加資料として渡す。

- `genspark-arare-ai-first-prompt-full-ja.md`
- `genspark-arare-ai-100-store-cost-rollout-prompt-ja.md`
- `genspark-arare-ai-voice-provider-adapter-prompt-ja.md`
